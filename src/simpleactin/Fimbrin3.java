 ...
 * @author sm2983
 */
public class Fimbrin3 implements SubUnitListener {

    final double KBT = 4.114;
    final static int GSCALE=2;
    ...
    public double _t = -1;
    private static GammaDistribution GammaDists[] = new GammaDistribution[110*GSCALE];
    ...
    public Fimbrin3(Filament f1, Filament f2, double k1, double k2, double kon1,
    ...
    public static void setDists(double Sh, double ShA, double Sc, double ScA) {
        for (int i = 0; i < GammaDists.length; i++) {
            GammaDists[i] = new GammaDistribution(Sh + ShA * i/(double)GSCALE, Sc + ScA * i/(double)GSCALE);
        }
    }
    ...
    public boolean update(double t) {
        if (_end1Connection + _end2Connection == 0) {
            ...
            double factor =  _k1*Math.exp(0*(GammaDists[(int) ((t-_t) * 10*GSCALE)].sample() / KBT));
            ...
            if (_rand.nextDouble() < k1p) {
             ...
             factor =  _k1*Math.exp(0*(GammaDists[(int) ((t-_t) * 10*GSCALE)].sample() / KBT));
            if (_rand.nextDouble() < k1p) {
                if (_detachEnd2) {
                    _detachEnd1 = true;
                } else {
            ...
            int n = _end1Connection == 1 ? _f2._subunits.size() : _f1._subunits.size();
            int subunit = 0;
            
            for (; subunit < 1 && !attach;) {
